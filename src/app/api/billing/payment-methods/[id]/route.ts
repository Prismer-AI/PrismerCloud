...
export async function PATCH(
  ...
  try {
    ...
    console.log(`[Billing] PATCH payment-method ${id}, useLocal=${useLocal}`);
    ...
    console.error('[Billing] PATCH payment-method error:', error);
    return NextResponse.json({
      success: false,
...
export async function DELETE(
  ...
  try {
    ...
    const useLocal = FEATURE_FLAGS.BILLING_LOCAL;
    console.log(`[Billing] DELETE payment-method ${id}, useLocal=${useLocal}`);
...
async function handlePatchLocal(
  ...
  console.log('[Billing] Using LOCAL implementation for PATCH');
  ...
  if (setDefault) {
    ...
    if (stripeCustomerId) {
      try {
        await setStripeDefault(stripeCustomerId, pm.stripe_payment_method_id);
      } catch (e) {
        console.warn('[Billing] Failed to set Stripe default:', e);
        // 不影响主流程
      }
    }
    
    console.log(`[Billing] Set payment method ${id} as default for user ${userId}`);
  }
  
  return NextResponse.json({
    success: true,
    message: setDefault 
      ? `Payment method ${id} set as default` 
      : `Payment method ${id} updated`
  });
}
...
async function handleDeleteLocal(
  ...
  console.log('[Billing] Using LOCAL implementation for DELETE');
  ...
  try {
    ...
    console.warn('[Billing] Failed to detach from Stripe:', e);
    // 继续删除本地记录
  }
  ...
  console.log(`[Billing] Deleted payment method ${id} for user ${userId}`);
...
async function handlePatchProxy(
  authHeader: string,
  id: string,
  setDefault: boolean
): Promise<NextResponse> {
  console.log('[Billing] Using PROXY implementation for PATCH');
...
async function handleDeleteProxy(
  ...
  console.log('[Billing] Using PROXY implementation for DELETE');
