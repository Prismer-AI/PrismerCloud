...
import { ConfigurationPanel, ApiPreviewPanel, ApiTabs, ParsePanel, IMPanel, type PlaygroundApi } from '@/components/playground';

const PRESETS = [
  { label: 'Efficient-VQGAN Paper', url: 'https://arxiv.org/html/2310.05400v1', strategy: Strategy.ACADEMIC },
  { label: 'Financial Report', url: 'https://investors.nvidia.com/financials/quarterly-results/FY24-Q3.pdf', strategy: Strategy.FINANCE },
  { label: 'Legal Contract', url: 'https://www.sec.gov/Archives/edgar/data/1318605/tsla-10k_2023.htm', strategy: Strategy.LEGAL },
];
...
function PlaygroundContent() {
  ...
  const handleSubmit = async (overrideUrl?: string) => {
    ...
    try {
      ...
      const data = await api.submitTask(targetInput, strategy, (content, done) => {
    ...
    <div className="max-w-[1600px] mx-auto px-4 sm:px-6 py-4 sm:py-8 min-h-[calc(100vh-64px)]">
      ...
      {showAuthPrompt && !isAuthenticated && (
        <div className="fixed inset-x-0 top-0 z-[100] flex justify-center pt-20 sm:pt-24 px-4 animate-in fade-in duration-300">
          ...
          />
          {/* Modal Card */}
          <div className={`relative border rounded-2xl p-5 sm:p-6 max-w-md w-full shadow-2xl animate-in slide-in-from-top-4 duration-500 ${isDark ? 'bg-zinc-900 border-white/10' : 'bg-white border-zinc-200'}`}>
            <button
              onClick={() => setShowAuthPrompt(false)}
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className={`text-sm ${isDark ? 'text-zinc-400' : 'text-zinc-600'}`}>Ready to integrate this into your app?</p>
              </div>
            </div>
            
            <p className={`text-sm mb-6 ${isDark ? 'text-zinc-400' : 'text-zinc-600'}`}>
              Create a free account to get your API key and start using Prismer Cloud&apos;s context extraction in your applications.
            </p>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 sm:mb-8 gap-4">
        <div className="flex items-center gap-3">
          <div className={`p-1.5 sm:p-2 bg-gradient-to-br from-violet-500/20 to-cyan-500/20 rounded-lg sm:rounded-xl border backdrop-blur-md ${isDark ? 'border-white/10' : 'border-violet-200'}`}>
            <Sparkles className="w-5 h-5 sm:w-6 sm:h-6 text-violet-400" />
          </div>
          <div>
            <h1 className={`text-xl sm:text-2xl md:text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r ${isDark ? 'from-white to-zinc-400' : 'from-zinc-900 to-zinc-600'}`}>
              {activeApi === 'context' ? 'Context Playground' : activeApi === 'parse' ? 'Parse Playground' : 'IM Playground'}
            </h1>
            <p className={`text-xs sm:text-sm mt-0.5 sm:mt-1 hidden sm:block ${isDark ? 'text-zinc-500' : 'text-zinc-600'}`}>
              {activeApi === 'context' ? 'Test ingestion strategies and preview HQCC outputs' : activeApi === 'parse' ? 'Extract text from PDFs and images' : 'Register users and send messages'}
            </p>
          </div>
        </div>
      </div>
      ...
      {activeApi === 'context' && <div className={`grid grid-cols-1 gap-4 sm:gap-6 lg:gap-8 relative transition-all duration-500 ease-out ${
        isPanelCollapsed ? 'xl:grid-cols-[64px_minmax(0,1fr)]' : 'xl:grid-cols-12'
      }`} style={{ alignItems: 'start' }}>
        {/* LEFT COLUMN (Input & Code) - Collapsible */}
        <div className={`z-20 transition-all duration-500 ease-out ${
          isPanelCollapsed ? 'xl:self-start' : 'xl:col-span-5 space-y-4 sm:space-y-6 xl:self-start'
        }`}>
          
          {/* Collapsed State - Floating Toggle Buttons */}
          {isPanelCollapsed && (
            <div className={`hidden xl:flex flex-col items-center gap-3 py-4 backdrop-blur-xl border rounded-2xl animate-in slide-in-from-left-4 fade-in duration-300 sticky top-24 ${isDark ? 'bg-zinc-900/90 border-white/10' : 'bg-white/90 border-zinc-200'}`}>
                    <button
          ...
          <div className={`space-y-4 sm:space-y-6 transition-all duration-500 ease-out ${
        ...
        <div className={`relative min-h-[400px] sm:min-h-[500px] lg:min-h-[600px] transition-all duration-500 w-full min-w-0 ${
          isPanelCollapsed ? '' : 'xl:col-span-7'
        }`}>
          
          {!result && !isProcessing && (
            <div className={`absolute inset-0 flex flex-col items-center justify-center border-2 border-dashed rounded-2xl sm:rounded-3xl opacity-50 p-4 ${isDark ? 'border-zinc-800' : 'border-zinc-300'}`}>
              <div className={`w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 rounded-full flex items-center justify-center mb-4 sm:mb-6 ${isDark ? 'bg-zinc-900' : 'bg-zinc-100'}`}>
                <Database className={`w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 ${isDark ? 'text-zinc-700' : 'text-zinc-400'}`} />
              </div>
              <p className={`font-medium text-sm sm:text-base ${isDark ? 'text-zinc-500' : 'text-zinc-600'}`}>Ready to Process</p>
              <p className={`text-xs sm:text-sm text-center ${isDark ? 'text-zinc-600' : 'text-zinc-500'}`}>Context results will appear here</p>
            </div>
          )}

          {/* Processing Loading State - Fixed at top */}
          {isProcessing && (
            <div className={`h-full rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col ${isDark ? 'bg-zinc-900 border border-white/5' : 'bg-white border border-zinc-200'}`}>
              {/* Fixed Loading Header */}
              <div className={`sticky top-0 z-10 px-6 py-8 ${isDark ? 'bg-zinc-900 border-b border-white/5' : 'bg-white border-b border-zinc-200'}`}>
                <div className="flex items-center gap-4">
                  <UniqueLoading variant="morph" size="md" />
                  <div>
                    <h3 className={`text-base font-semibold ${isDark ? 'text-white' : 'text-zinc-900'}`}>Processing Content</h3>
                    <p className={`text-sm ${isDark ? 'text-zinc-400' : 'text-zinc-600'}`}>Searching and extracting context...</p>
                  </div>
                </div>
                {/* Progress bar */}
              ...
              <div className="flex-1 overflow-auto p-4 sm:p-6 lg:p-8">
                {streamingContent ? (
                  <div className={`prose max-w-none animate-in fade-in duration-300 ${isDark ? 'prose-invert prose-zinc' : 'prose-zinc'}`}>
                    <MarkdownRenderer content={streamingContent} />
                  </div>
                ) : (
                  <div className="flex flex-col items-center justify-center h-full text-center py-12">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-4 animate-pulse ${isDark ? 'bg-zinc-800' : 'bg-zinc-100'}`}>
          ...
          {/* Results Container */}
          {result && (
            <div className={`h-full rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col animate-in slide-in-from-left-8 fade-in duration-500 ease-out fill-mode-forwards origin-left ${isDark ? 'bg-zinc-900 border border-white/5' : 'bg-white border border-zinc-200'}`}>
              ...
              {result.inputType === 'query' && result.sources && result.sources.length > 1 && (
                <div className={`px-3 sm:px-4 py-2 flex items-center gap-2 sm:gap-3 ${isDark ? 'bg-zinc-950/80 border-b border-white/5' : 'bg-zinc-50 border-b border-zinc-200'}`}>
                  ...
                  <div className="flex items-center gap-1 text-zinc-500">
                    ...
                    <span className="text-[10px] sm:text-xs font-mono">{activeSourceIndex + 1}/{result.sources.length}</span>
                    <button
                      onClick={() => handleSourceSwitch(Math.min((result.sources?.length || 1) - 1, activeSourceIndex + 1))}
              ...
              {/* Tabs Header */}
              <div className={`flex flex-col sm:flex-row sm:items-center justify-between px-2 pt-2 gap-2 sm:gap-0 ${isDark ? 'bg-black/20 border-b border-white/5' : 'bg-zinc-100/50 border-b border-zinc-200'}`}>
                <div className="flex gap-0.5 sm:gap-1 overflow-x-auto pb-1 sm:pb-0 scrollbar-none">
                  <button
                    ...
                    className={`px-2 sm:px-4 py-2 sm:py-3 rounded-t-lg text-[10px] sm:text-xs font-bold uppercase tracking-wider flex items-center gap-1 sm:gap-2 transition-all whitespace-nowrap
                      ${activeTab === 'hqcc'
                        ? (isDark ? 'bg-zinc-900 text-white border-t border-x border-white/5 shadow-[-1px_-1px_0_rgba(255,255,255,0.05)]' : 'bg-white text-zinc-900 border-t border-x border-zinc-200 shadow-sm')
                        : (isDark ? 'text-zinc-500 hover:text-zinc-300 hover:bg-white/5' : 'text-zinc-500 hover:text-zinc-700 hover:bg-zinc-200/50')
                      }`}
                  >
                    <FileType className="w-3 h-3 sm:w-4 sm:h-4" /> <span className="hidden xs:inline">HQCC</span><span className="xs:hidden">HQ</span>
                  </button>
                  <button
                ...
                <div className="flex items-center gap-2 sm:gap-3 pr-2 sm:pr-4 pb-1">
                  ...
                  <div className="hidden sm:flex items-center gap-1.5 px-2 sm:px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[9px] sm:text-[10px] font-bold uppercase tracking-wide">
                    <CheckCircle2 className="w-2.5 h-2.5 sm:w-3 sm:h-3" /> <span className="hidden md:inline">Processed in</span> {((result.json as { processing_time_ms?: number })?.processing_time_ms || 0) / 1000}s
                  </div>
              ...
              {(() => {
                ...
                return (
                  <div className={`flex flex-wrap items-center gap-x-3 gap-y-1 px-4 py-2 text-[10px] sm:text-xs ${isDark ? 'bg-zinc-950/50 border-b border-white/5' : 'bg-zinc-50/80 border-b border-zinc-200'}`}>
                    {/* Cache badge */}
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md font-medium ${
                      ...
                      {isCached ? <><Database className="w-3 h-3" /> Cache HIT</> : <><CheckCircle2 className="w-3 h-3" /> Processed</>}
                    </span>
                    {/* Mode */}
                    <span className={isDark ? 'text-zinc-500' : 'text-zinc-500'}>{mode}</span>
                    {/* Processing time */}
                    <span className={isDark ? 'text-zinc-400' : 'text-zinc-600'}>{(processingMs / 1000).toFixed(1)}s</span>
                    {/* Compression ratio */}
                    {compressionPct > 0 && (
              ...
              {/* Content Area */}
              <div className={`flex-1 overflow-hidden relative flex flex-col lg:flex-row ${isDark ? 'bg-zinc-900' : 'bg-white'}`}>
                  ...
                  <div className="absolute inset-0 z-50 bg-zinc-900/98 backdrop-blur-sm animate-in fade-in duration-200">
                    ...
                    <div className="sticky top-0 px-6 py-6 bg-zinc-900 border-b border-white/5">
                      ...
                      <div className="mt-4 h-0.5 bg-zinc-800 rounded-full overflow-hidden">
                        <div className="h-full w-full bg-gradient-to-r from-violet-500 via-cyan-500 to-violet-500 animate-pulse" 
                ...
                {activeTab === 'hqcc' && (
                  <>
                    ...
                    {result.inputType === 'query' && result.sources && result.sources.length > 1 && (
                      <div className={`absolute top-0 left-0 right-0 z-10 px-4 py-2 ${isDark ? 'bg-gradient-to-b from-zinc-900 via-zinc-900/95 to-transparent' : 'bg-gradient-to-b from-white via-white/95 to-transparent'}`}>
                        <a 
                    ...
                    {/* Main Content */}
                    <div className={`flex-1 overflow-auto p-4 sm:p-6 lg:p-8 ${result.inputType === 'query' && result.sources && result.sources.length > 1 ? 'pt-10' : ''}`}>
                  <div className={`prose max-w-none ${isDark ? 'prose-invert prose-zinc' : 'prose-zinc'}`}>
                        {isProcessing && streamingContent ? (
                          <MarkdownRenderer content={streamingContent} />
                        ) : (
                          <MarkdownRenderer 
                            content={result.sources?.[activeSourceIndex]?.hqcc || result.hqcc} 
                          />
                        )}
                      </div>
                    </div>
                    ...
                    <div className="group/toc absolute right-0 top-0 bottom-0 z-20 hidden xl:flex">
                      ...
                      <div className="w-8 h-full flex items-start justify-center pt-4 cursor-pointer">
                        <div className={`w-1 h-16 rounded-full group-hover/toc:opacity-0 transition-opacity ${isDark ? 'bg-white/10' : 'bg-zinc-300'}`} />
                            </div>
                      {/* TOC Panel */}
                      <div className={`w-56 border-l backdrop-blur-sm overflow-y-auto 
                        ...
                        <div className="p-4 sticky top-0">
                          <h4 className={`text-[10px] font-bold uppercase tracking-wider mb-3 flex items-center gap-2 ${isDark ? 'text-zinc-500' : 'text-zinc-600'}`}>
...
export default function PlaygroundPage() {
  return (
    <Suspense fallback={<div className="flex h-[calc(100vh-64px)] items-center justify-center"><Loader2 className="w-8 h-8 animate-spin text-violet-500" /></div>}>
      <PlaygroundContent />
    </Suspense>
  );
}
