...
export function createContactsRouter() {
  ...
  router.get("/", async (c) => {
    ...
    for (const p of otherParticipants) {
      ...
      if (
        ...
        (p.conversation.lastMessageAt &&
          (!existing.conversation.lastMessageAt ||
            p.conversation.lastMessageAt > existing.conversation.lastMessageAt))
      ) {
    ...
    const cursorMap = new Map(
      readCursors.map((rc: { conversationId: string; lastReadAt: Date }) => [rc.conversationId, rc.lastReadAt] as const)
    );
...
export function createDiscoverRouter() {
   ...
   *   q           - Search by username or displayName
   *   limit       - Max results (default 50)
   *   offset      - Skip N results (default 0)
   */
  router.get("/", async (c) => {
