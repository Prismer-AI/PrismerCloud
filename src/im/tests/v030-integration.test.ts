...
async function createTestUser(
  ...
  if (!res.data.ok) {
    ...
    if (!loginRes.data.ok) {
      throw new Error(
        `Cannot create/login user ${username}: ${JSON.stringify(res.data)}`
      );
    }
    return {
...
async function testBindings() {
  ...
  await test("B1: Create Telegram binding", async () => {
    ...
    assert(
      res.data.data.verificationCode.length === 6,
  ...
  await test("B2: Duplicate binding same platform → 409", async () => {
    const res = await api(
      "POST",
      "/bindings",
      { platform: "telegram", botToken: "another-token" },
      userAToken
    );
  ...
  await test("B3: Verify binding with correct code", async () => {
    ...
    assertEqual(res.data.data.platform, "telegram", "platform");
    assert(
      Array.isArray(res.data.data.capabilities),
  ...
  await test("B4: Verify binding with wrong code → 400", async () => {
    ...
    const res = await api(
      "POST",
      `/bindings/${discordBindingId}/verify`,
      { code: "000000" },
      userAToken
    );
    ...
    assert(!res.data.ok, "ok should be false");
    assert(
      res.data.error.includes("Invalid verification"),
  ...
  await test("B6: Revoke binding", async () => {
    ...
    const createRes = await api(
      "POST",
      "/bindings",
      { platform: "slack", botToken: "slack-test" },
      userAToken
    );
  ...
  await test("B12: Binding not found → 404", async () => {
    const res = await api(
      "POST",
      "/bindings/nonexistent_id/verify",
      { code: "123456" },
      userAToken
    );
  ...
  await test("B14: Re-verify active binding → 400", async () => {
    ...
    assertEqual(res.status, 400, "status");
    assert(
      res.data.error.includes("cannot verify"),
      "should say cannot verify"
    );
  });
...
async function testCredits() {
  ...
  await test("C1: New user default balance = 10000 (≈10M messages)", async () => {
    const res = await api("GET", "/credits", undefined, userAToken);
    assertEqual(res.status, 200, "status");
    assert(res.data.ok, "ok should be true");
    assertEqual(res.data.data.balance, 10000, "default balance");
    assert(
      res.data.data.totalEarned === 0 || res.data.data.totalEarned === 10000,
      `totalEarned should be 0 or 10000, got ${res.data.data.totalEarned}`
    );
  ...
  await test("C3: Transaction history — initially empty", async () => {
    ...
    assert(
      ...
      `expected 0 or 1 initial txns, got ${res.data.data.length}`
    );
    assert(res.data.meta.total <= 1, `total should be 0 or 1, got ${res.data.meta.total}`);
  });

  await test("C4: UserB also gets default 10000 credits", async () => {
  ...
  await test("C6: Transactions with limit/offset", async () => {
    const res = await api(
      "GET",
      "/credits/transactions?limit=5&offset=0",
      undefined,
      userAToken
    );
    assertEqual(res.status, 200, "status");
  ...
  await test("C7: Limit capped at 100", async () => {
    const res = await api(
      "GET",
      "/credits/transactions?limit=999",
      undefined,
      userAToken
    );
    assertEqual(res.status, 200, "status");
  ...
  await test("C8: Users have isolated credits", async () => {
    ...
    assertEqual(resA.data.data.balance, 10000, "userA balance");
    assertEqual(resB.data.data.balance, 10000, "userB balance");
  });
}
...
async function testCreditServiceInternal() {
  ...
  const { LocalCreditService } = await import(
  ...
  await test("CS1: ensureCredit creates record", async () => {
    ...
    assertEqual(balance.balance, 10000, "initial balance");
  });
  ...
  await test("CS2: deduct reduces balance", async () => {
    ...
    assertEqual(result.balanceAfter, 9999.5, "balance after");
  });

  await test("CS3: deduct records transaction", async () => {
    const { transactions, total } = await creditService.getTransactions(
      userId,
      10,
      0
    );
    ...
    assertEqual(transactions[0].balanceAfter, 9999.5, "balanceAfter");
  ...
  await test("CS4: credit adds balance", async () => {
    ...
    assertEqual(result.balanceAfter, 10009.5, "balance after credit");
  });

  await test("CS5: balance reflects both deduct and credit", async () => {
    const balance = await creditService.getBalance(userId);
    assertEqual(balance.balance, 10009.5, "balance");
  ...
  await test("CS6: insufficient credits → failure", async () => {
    const result = await creditService.deduct(userId, 999, "too much");
    assert(!result.success, "should fail");
    assert(result.error?.includes("Insufficient"), "error message");
    assertEqual(result.balanceAfter, 10009.5, "balance unchanged");
  });
  ...
  await test("CS7: multiple deductions sequential", async () => {
    ...
    const balance = await creditService.getBalance(userId);
    const expected = 10009.5 - 5 * 0.001;
    assert(
      Math.abs(balance.balance - expected) < 0.0001,
      `balance should be ~${expected}, got ${balance.balance}`
    );
  });
  ...
  await test("CS9: transaction pagination offset", async () => {
    ...
    assert(
      page1.transactions[0].id !== page2.transactions[0].id,
...
async function testBridge() {
  ...
  const { BridgeManager } = await import(
    "../services/bridge/bridge-manager"
  );
  ...
  await test("BR8: Bridge message record — outbound with no active bindings", async () => {
    ...
    await bridgeManager.sendOutbound(
      testUser.id,
      "test message",
      "msg_fake",
      "conv_fake"
...
async function testMeEnhancement() {
  ...
  await test("ME1: /me includes bindings array", async () => {
    ...
    assert(res.data.ok, "ok");
    assert(
      Array.isArray(res.data.data.bindings),
      "bindings should be array"
    );
    // userA has telegram (active) + discord (pending)
    assert(
      res.data.data.bindings.length >= 1,
      "should have at least 1 binding"
    );
    const tg = res.data.data.bindings.find(
      (b: any) => b.platform === "telegram"
    );
    assert(tg, "should have telegram binding");
    assertEqual(tg.status, "active", "telegram status");
  });
  ...
  await test("ME2: /me includes credits", async () => {
    ...
    assertEqual(res.data.data.credits.balance, 10000, "default balance");
  });
}
...
async function main() {
  ...
  try {
    ...
    console.error("   Start the server: DATABASE_URL=\"file:/Users/prismer/workspace/prismercloud/prisma/dev.db\" npx tsx src/im/start.ts");
    process.exit(1);
  }
