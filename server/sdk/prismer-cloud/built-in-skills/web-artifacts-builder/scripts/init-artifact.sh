...
# Check if pnpm is installed. Release 201 sandbox image bakes pnpm@9.
# If pnpm is missing the image is out of date (or we're in a dev env);
# try corepack as a per-user fallback (no sudo), else fail loudly so the
# agent surfaces the missing-baseline error rather than installing pnpm
# at task time and contaminating the workdir.
if ! command -v pnpm &> /dev/null; then
  ...
  if command -v corepack &> /dev/null; then
    ...
    echo "❌ Error: pnpm is required but neither pnpm nor corepack is available."
    echo "   Sandbox image is missing pnpm — rebuild the daemon image with the"
    echo "   release 201 baseline (infra/sandbox-image/Dockerfile.daemon)."
    exit 1
  fi
fi
