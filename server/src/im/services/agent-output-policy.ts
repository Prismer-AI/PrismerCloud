...
const RULES: AgentOutputRule[] = [
  ...
  },
  {
    ...
    mimeAllowed: (mime) => OFFICE_MIMES.has(mime),
  },
  {
    category: 'image',
    maxBytes: 25 * MB,
  ...
  },
  {
    ...
    mimeAllowed: (mime) => mime === 'application/zip',
  },
];
...
export function validateAgentOutputAsset(input: {
  ...
  const mime = normalizeMime(input.mime) ?? MIME_BY_EXTENSION[ext] ?? null;
  ...
  if (mime === 'application/octet-stream')
    return { ok: false, reason: 'agent output MIME application/octet-stream is not allowed' };
  ...
  if (mime.startsWith('image/') && mime !== 'image/svg+xml' && !RASTER_IMAGE_MIMES.has(mime)) {
    return { ok: false, reason: `agent output image MIME ${mime} is not allowed; use SVG or HTML` };
  }
