...
async function main() {
  ...
  console.log(`  Users: A=${userAId.slice(0, 8)}… B=${userBId.slice(0, 8)}… X=${agentXId.slice(0, 8)}… Y=${agentYId.slice(0, 8)}…`);
  ...
  await test("W5: Init group workspace with users + agents", async () => {
    const res = await api("POST", "/workspace/init-group", {
  ...
  await test("W12: Add agent to workspace", async () => {
    const res = await api("POST", `/workspace/ws_${TS}_1/agents`, {
  ...
  await test("W17: Mentions autocomplete", async () => {
    const res = await api("GET", `/workspace/mentions/autocomplete?conversationId=${workspaceConvId}&query=&limit=10`, undefined, userAToken);
    assertEqual(res.status, 200, "status");
    assert(res.data.ok, "ok");
    assert(Array.isArray(res.data.data), "is array");
  });
  ...
  await test("A9: Agent sends heartbeat", async () => {
    const res = await api("POST", `/agents/${agentXId}/heartbeat`, {
      status: "online",
      load: 0.3,
  ...
  await test("MSG1: Send text message", async () => {
    const res = await api("POST", `/messages/${directConvId}`, {
  ...
  await test("MSG12: Edit own message", async () => {
    const res = await api("PATCH", `/messages/${directConvId}/${testMessageId}`, {
  ...
  await test("MSG15: Edit non-existent message", async () => {
    const res = await api("PATCH", `/messages/${directConvId}/nonexistent_id`, {
      content: "Ghost edit",
    }, userAToken);
    assertEqual(res.status, 404, "status");
  });
  ...
  await test("D1: Send direct message by username", async () => {
    const res = await api("POST", `/direct/comp_ub_${TS}/messages`, {
  ...
  await test("D3: Cannot message yourself", async () => {
    const res = await api("POST", `/direct/${userAId}/messages`, {
      content: "Hello me!",
    }, userAToken);
    assertEqual(res.status, 400, "status");
  ...
  await test("D4: Message non-existent user", async () => {
    const res = await api("POST", "/direct/totally_nonexistent_user/messages", {
      content: "Hello ghost!",
    }, userAToken);
    assertEqual(res.status, 404, "status");
  });
  ...
  await test("G9: Send message to group", async () => {
    const res = await api("POST", `/groups/${groupConvId}/messages`, {
      content: "Hello group!",
    }, userAToken);
    assertEqual(res.status, 201, "status");
    assert(res.data.ok, "ok");
  ...
  await test("G15: Owner adds member", async () => {
    const res = await api("POST", `/groups/${groupConvId}/members`, {
      userId: agentYId,
    }, userAToken);
    assertEqual(res.status, 201, "status");
    assert(res.data.ok, "ok");
  ...
  await test("G17: Non-admin cannot add members", async () => {
    ...
    const res = await api("POST", `/groups/${groupConvId}/members`, {
      userId: extra.id,
    }, userBToken); // userB is 'member', not owner/admin
    assertEqual(res.status, 403, "status");
  });
  ...
  await test("CV12: Archive conversation", async () => {
    ...
    const tempGroup = await api("POST", "/groups", {
      title: "Archive Me",
    }, userAToken);
  ...
  await test("E4: Duplicate username from same registration type", async () => {
    ...
    assert(res2.status === 200 || res2.status === 201 || res2.status === 409,
  ...
  await test("I3: Workspace API → Messages API consistency", async () => {
    ...
    const sendRes = await api("POST", `/messages/${wsConvId}`, {
  ...
  await test("I4: Full lifecycle: register → send → read → unread", async () => {
    ...
    const sendRes = await api("POST", `/direct/${userAId}/messages`, {
