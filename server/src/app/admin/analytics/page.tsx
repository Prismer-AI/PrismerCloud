...
import { SandboxFleetSection } from './sandbox-section';
            ...
            <>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
                  ...
                  <StatCard
                    label="API Keys"
                    value={`${data.overview.activeApiKeys} / ${data.overview.totalApiKeys}`}
                    sub="active / total"
                    icon={Zap}
                  />
                  <StatCard
                    label="Credit Balance"
                    value={formatNum(data.overview.totalBalance)}
                    sub="platform total"
                    icon={Database}
                  />
                  <StatCard
                    label="Credits Used (30d)"
                    value={formatNum(data.overview.creditsUsed30d)}
                    icon={TrendingUp}
                  />
                  <StatCard label="Revenue (30d)" value={formatUSD(data.overview.revenue30d)} icon={CreditCard} />
                  <StatCard
                    label="IM Agents"
                    value={formatNum(data.im.totalAgents)}
                    sub={`${data.im.totalUsers} total IM users`}
                    icon={Bot}
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-4">
                  ...
                  <StatCard
                    ...
                    value={formatNum(data.im.activeConversations)}
                    sub={`in last ${period}`}
                    icon={Activity}
                  />
                  {data.imDetail && (
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
                    ...
                    <StatCard
                      label="Evo Success"
                      value={
                        data.features.evolutionCapsules.total > 0
                          ? formatPct(
                              (data.features.evolutionCapsules.success / data.features.evolutionCapsules.total) * 100,
                            )
                          : 'N/A'
                      }
                ...
                return (
                  <div>
                    ...
                    <div className="space-y-3">
                      <span
                        className={`px-2 py-0.5 rounded text-[10px] font-medium ${verdictColors[v] || verdictColors.insufficient_data}`}
                      >
                        {verdictLabels[v] || v}
                      </span>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {['standard', 'hypergraph'].map((mode) => {
                          const m = (evo as Record<string, unknown>)[mode] as
                            | { ssr?: number; rp?: number; gd?: number; er?: number; totalCapsules?: number }
                            | undefined;
                          return (
                            <div key={mode} className="p-3 rounded-lg bg-zinc-100 dark:bg-zinc-800/50">
                              <div className="text-xs font-medium mb-2 text-zinc-600 dark:text-zinc-300 capitalize">
                                {mode} Mode
                              </div>
                              {m && m.totalCapsules ? (
                                <div className="grid grid-cols-2 gap-2">
                                  ...
                                  <StatCard
                                    ...
                                    value={m.rp != null ? formatPct(m.rp * 100) : 'N/A'}
                                  />
              ...
              {/* ============================================================ */}
              {/* Section 5.6: Evolution Funnel (Pre-A Metrics) */}
              {/* ============================================================ */}
              {data.features && (
                <div>
                  <SectionTitle>Evolution Funnel (Pre-A Metrics)</SectionTitle>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                    <StatCard label="Agents Evolving" value={formatNum(data.features.evolutionAgents)} icon={Brain} />
                    <StatCard
                      label="ERR (Approx)"
                      value={(() => {
                        const evo = (data as unknown as Record<string, unknown>).evolutionExperiment as
                          | Record<string, any>
                          | undefined;
                        const err = evo?.standard?.errApprox;
                        return err != null ? (err > 0 ? '+' : '') + formatPct(err * 100) : 'collecting...';
                      })()}
                      sub="Error Reduction Rate"
                      icon={TrendingUp}
                    />
                    <StatCard
                      label="Cross-Agent Adoption"
                      value={(() => {
                        const capsules = data.features.evolutionCapsules;
                        return capsules.total > 0 ? formatNum(capsules.total) + ' capsules' : 'N/A';
                      })()}
                      sub="Gene executions across agents"
                      icon={Activity}
                    />
                    <StatCard
                      label="Success Rate"
                      ...
                      sub="Global gene success rate"
                      icon={CheckCircle}
                    />
                  </div>
                  <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5">
                    <h3 className="text-sm font-medium text-zinc-500 dark:text-zinc-400 mb-3">
                      Evolution Pipeline Funnel
                    </h3>
                    <div className="space-y-2 text-sm">
                      {[
                        { label: 'Agents Registered', value: data.im.totalAgents, width: 100 },
                        {
                          label: 'Agents with Capsules',
                          value: data.features.evolutionAgents,
                          width:
                            data.im.totalAgents > 0 ? (data.features.evolutionAgents / data.im.totalAgents) * 100 : 0,
                        },
                        { label: 'Total Capsules', value: data.features.evolutionCapsules.total, width: 80 },
                        {
                          label: 'Successful Outcomes',
                          value: data.features.evolutionCapsules.success,
                          width:
                            data.features.evolutionCapsules.total > 0
                              ? (data.features.evolutionCapsules.success / data.features.evolutionCapsules.total) * 80
                              : 0,
                        },
                      ].map((step) => (
                        <div key={step.label} className="flex items-center gap-3">
                          <span className="w-40 text-zinc-500 dark:text-zinc-400 shrink-0">{step.label}</span>
                          <div className="flex-1 h-6 bg-zinc-100 dark:bg-zinc-800 rounded overflow-hidden">
                            <div
                              className="h-full bg-purple-500/30 rounded flex items-center px-2"
                        ...
                        {data.realtime.endpoints.map((ep) => (
                          <tr key={ep.endpoint} className="border-b border-zinc-100 dark:border-zinc-800 last:border-0">
                            ...
                            <td className="px-5 py-3 text-sm text-right font-mono">
                              {ep.errorCount} ({formatPct(ep.errorRate * 100)})
                            </td>
                          </tr>
                        ))}
                      </tbody>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                  <StatCard
                    label="Revenue"
                    value={formatUSD(data.revenue.totalRevenue)}
                    sub={`in last ${period}`}
                    icon={CreditCard}
                  />
                        ...
                        <BarChart data={data.revenue.chart}>
                          ...
                          <Tooltip
                            ...
                            formatter={(value: any, name: any) => [
                              name === 'revenue' ? formatUSD(Number(value)) : formatNum(Number(value)),
                              name,
                            ]}
                          />
                          <Legend wrapperStyle={{ fontSize: 12 }} />
              ...
              {/* ============================================================ */}
              {/* Section 10: Sandbox Fleet (Release 200 §08 §7.1)
                  Renders v200 sandbox observability inside this page so
                  /admin/sandbox stays the legacy SRE console. Fail-soft:
                  the inner hook handles its own fetch errors. */}
              {/* ============================================================ */}
              <div>
                <SectionTitle>Sandbox Fleet (v200)</SectionTitle>
                <SandboxFleetSection />
              </div>
