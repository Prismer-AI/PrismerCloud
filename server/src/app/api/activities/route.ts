...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!authHeader) {
      metrics.recordRequest('/api/activities', Date.now() - reqStart, 401);
    ...
    log.debug({ FF_ACTIVITIES_LOCAL: process.env.FF_ACTIVITIES_LOCAL, useLocal }, 'Feature flag check');

    let result: NextResponse;
    if (useLocal) {
      result = await handleActivitiesLocal(authHeader, page, limit);
    } else {
      result = await handleActivitiesProxy(authHeader, page, limit);
    }
    metrics.recordRequest('/api/activities', Date.now() - reqStart, result.status);
    return result;
  } catch (error) {
    log.error({ err: error }, 'Activities error');
    metrics.recordRequest('/api/activities', Date.now() - reqStart, 500);
    return NextResponse.json(
      {
        success: false,
 ...
 * 本地实现：直连数据库
 */
async function handleActivitiesLocal(authHeader: string, page: number, limit: number): Promise<NextResponse> {
  ...
  log.info({ userId, found: activities.length, total }, 'Activities loaded');
 ...
 * 代理实现：转发到后端
 */
async function handleActivitiesProxy(authHeader: string, page: number, limit: number): Promise<NextResponse> {
