 ...
 */
function truncateContent(content: string, maxChars: number = MAX_CONTENT_CHARS): { content: string; truncated: boolean; originalLength: number } {
  ...
  truncatedContent += '\n\n[Content truncated due to length limit. Original content was ' + 
    Math.round(originalLength / 1000) + 'k characters, truncated to ' + 
    Math.round(truncatedContent.length / 1000) + 'k characters for processing.]';
  
  console.log(`[Compress] Content truncated: ${originalLength} -> ${truncatedContent.length} chars (estimated ${Math.round(originalLength / CHARS_PER_TOKEN)}k -> ${Math.round(truncatedContent.length / CHARS_PER_TOKEN)}k tokens)`);
 ...
 * - strategy: string - Compression strategy (Academic, Business, Technical, etc.)
 * - imageLinks: string[] - Optional array of image URLs to include
 * - stream: boolean - If true, returns streaming response
 * 
 * Response:
 * - hqcc: string - Compressed High-Quality Curated Context (non-streaming)
 * - Or: text/event-stream with chunks (streaming)
 */
