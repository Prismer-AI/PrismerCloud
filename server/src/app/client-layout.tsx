...
export function ClientLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div
      className={`min-h-screen font-sans bg-grid relative transition-colors flex flex-col ${
    ...
    >
      ...
      {!isLanding && (
        <div className="fixed inset-0 z-0">
          ...
          />
          <div
            className={`absolute top-1/2 right-0 w-[400px] h-[400px] rounded-full blur-[100px] ${
              isDark ? 'bg-blue-600/5' : 'bg-blue-400/8'
            }`}
          />
          {/* Noise overlay for texture */}
          <div
            className={`absolute inset-0 bg-[url('data:image/svg+xml,%3Csvg viewBox=%270 0 256 256%27 xmlns=%27http://www.w3.org/2000/svg%27%3E%3Cfilter id=%27noise%27%3E%3CfeTurbulence type=%27fractalNoise%27 baseFrequency=%270.8%27 numOctaves=%274%27 stitchTiles=%27stitch%27/%3E%3C/filter%3E%3Crect width=%27100%25%27 height=%27100%25%27 filter=%27url(%23noise)%27/%3E%3C/svg%3E')] ${
      ...
      <main className={`relative z-10 flex-1 pt-[88px] ${isWorkspace ? 'min-h-0 overflow-hidden' : 'pb-12'}`}>
        {children}
      </main>
