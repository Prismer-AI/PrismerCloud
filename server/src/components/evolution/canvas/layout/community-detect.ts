...
export function detectDomains(
  ...
  if (nonZeroCount < 20) {
    return coldStartFallback(edges, genes, signals);
  }
  ...
  while (improved && iteration < MAX_ITERATIONS) {
    ...
    for (const nodeId of geneIds) {
      ...
      for (const [candidateComm, kiCandidate] of communityEdgeWeight) {
        ...
        const gain =
          (kiCandidate - kiIn) / m2 -
          resolution * ki * (sigmaTotCandidate - sigmaTotCurrent + ki) / (m2 * m2);
      ...
      if (bestCommunity !== currentCommunity) {
        ...
        communityInternalWeight.set(
          currentCommunity,
          (communityInternalWeight.get(currentCommunity) ?? 0) - 2 * kiIn,
        );
        ...
        communityInternalWeight.set(
          bestCommunity,
          (communityInternalWeight.get(bestCommunity) ?? 0) + 2 * kiBest,
        );
    ...
    while (superImproved && superIter < 20) {
      ...
      for (const sn of superNodes) {
        ...
        for (const [candidateComm, kiCandidate] of commWeights) {
          ...
          const gain =
            (kiCandidate - kiIn) / sm2 -
            resolution * ki * (sigmaCandidate - sigmaCurrent + ki) / (sm2 * sm2);
          if (gain > bestGain) {
            bestGain = gain;
