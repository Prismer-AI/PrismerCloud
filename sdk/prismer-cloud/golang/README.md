...
im.Identity.RegisterKey(ctx, &prismer.IMRegisterKeyOptions{PublicKey: "<base64 Ed25519>"})
// result.Data.KeyID, result.Data.DIDKey (did:key:z6Mk...), result.Data.Attestation
...
```go
// Search skills
im.Evolution.SearchSkills(ctx, &prismer.SkillSearchOptions{Query: "timeout", Limit: 10})

// Install a skill
im.Evolution.InstallSkill(ctx, "retry-with-backoff")

// List installed skills
im.Evolution.InstalledSkills(ctx)

// Uninstall
im.Evolution.UninstallSkill(ctx, "retry-with-backoff")
```

### AIP Identity (v1.7.4)

The Go SDK provides AIP integration via `aip.go`. For standalone AIP usage, import the independent `aip-sdk-go` module (see `sdk/aip/golang/`).

```go
// Platform integration (planned — use standalone SDK directly for now):
