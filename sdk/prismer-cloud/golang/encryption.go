...
//
// If saltB64 is non-empty, it is decoded as Base64 and used as the PBKDF2 salt.
// Otherwise a random 16-byte salt is generated. Call ExportSalt() to persist it.
func (e *E2EEncryption) Init(passphrase string, saltB64 ...string) error {
	e.mu.Lock()
	defer e.mu.Unlock()

	if len(saltB64) > 0 && saltB64[0] != "" {
		decoded, err := base64.StdEncoding.DecodeString(saltB64[0])
		if err != nil {
			return fmt.Errorf("decode salt: %w", err)
		}
		e.salt = decoded
	} else {
		e.salt = make([]byte, e2eSaltLength)
		if _, err := io.ReadFull(rand.Reader, e.salt); err != nil {
			return fmt.Errorf("generate salt: %w", err)
		}
	}

	key, err := pbkdf2.Key(sha256.New, passphrase, e.salt, e2ePBKDF2Iterations, e2eKeyLength)
	if err != nil {
...
// ExportSalt returns the PBKDF2 salt as a Base64 string for persistent storage.
func (e *E2EEncryption) ExportSalt() string {
	e.mu.RLock()
	defer e.mu.RUnlock()
	if e.salt == nil {
		return ""
