...
export function register(parent: Command, getIMClient: ClientFactory, _getAPIClient: ClientFactory): void {
  ...
  // ---------------------------------------------------------------------------
  // 14. evolve publish <gene-id>
  // ---------------------------------------------------------------------------
  evolve
    .command('publish <gene-id>')
    .description('Publish a private gene to the evolution network')
    .option('--skip-canary', 'skip canary phase and publish directly')
    .option('--json', 'output raw JSON response')
    .action(async (geneId: string, opts: { skipCanary?: boolean; json?: boolean }) => {
      const client = getIMClient();
      try {
  ...
  // ---------------------------------------------------------------------------
  // 15. evolve fork <gene-id>
  // ---------------------------------------------------------------------------
  evolve
    .command('fork <gene-id>')
    .description('Fork a public gene with optional modifications')
    .option('--strategy <steps...>', 'override strategy steps')
    .option('--json', 'output raw JSON response')
    .action(async (geneId: string, opts: { strategy?: string[]; json?: boolean }) => {
      const client = getIMClient();
      try {
        const res = await client.im.evolution.forkGene({
          gene_id: geneId,
          modifications: opts.strategy ? { strategy: opts.strategy } : undefined,
        });
        ...
        printResult(res);
        const data = res.data as { id?: string; gene_id?: string } | undefined;
  ...
  // ---------------------------------------------------------------------------
  // 16. evolve delete <gene-id>
  // ---------------------------------------------------------------------------
  evolve
    .command('delete <gene-id>')
    .description('Delete a gene you own')
    .option('--json', 'output raw JSON response')
