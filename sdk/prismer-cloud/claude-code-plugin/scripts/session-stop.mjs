...
function hasEvolutionValue(text) {
  ...
  // Without signals, there is nothing evolution-relevant to record — only prismer.memory.write
...
function buildContext(text) {
  ...
  if (feedbacks.length > 0) {
    ...
    for (const fb of feedbacks) {
      ...
      parts.push(`    Call prismer.evolve.record with YOUR assessment of outcome + whether you used the strategy`);
    }
  }
  ...
  // Concise instructions: only suggest prismer.evolve.record/prismer.evolve.createGene when there is
  // gene feedback or repeated signals (evolution-relevant). Otherwise only prismer.memory.write.
  const hasGeneFeedback = feedbacks.length > 0;
  const hasRepeatedSignals = sigList.length > 0;

  if (parts.length === 0) {
    // No gene feedback and no repeated signals — only prismer.memory.write is relevant
    parts.push('Session had activity. Review: prismer.memory.write (project-specific learnings). Max 1 call.');
  } else if (hasGeneFeedback || hasRepeatedSignals) {
    parts.push('');
    parts.push('Review: prismer.evolve.record (gene feedback) / prismer.evolve.createGene (general pattern) / prismer.memory.write (project-specific). Max 3 calls.');
  } else {
    parts.push('');
    parts.push('Review: prismer.memory.write (project-specific learnings). Max 1 call.');
  }
