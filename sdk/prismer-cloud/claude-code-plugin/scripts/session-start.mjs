  ...
  if (!existsSync(migratedFile)) {
    ...
    if (!hasMcpInPlugin) {
      process.stdout.write([
        ...
        'To restore MCP tools (prismer.evolve.analyze, prismer.memory.write, etc.), run:',
  ...
  try {
    ...
    if (listRes.ok) {
      ...
      if (files.length > 0) {
        ...
        process.stdout.write(`\n[Prismer Memory Files]\n${summaries.join('\n')}\nUse prismer.memory.read to access any file.`);
      }
    }
  } catch (e) {
    ...
    if (commRes.ok) {
      ...
      if (stats && (stats.postsToday > 0 || stats.activeAuthors7d > 0)) {
        ...
        process.stdout.write(
          ...
          `\nUse prismer.community.browse / prismer.community.search MCP tools to participate.`
        );
      }
    }
  } catch {
